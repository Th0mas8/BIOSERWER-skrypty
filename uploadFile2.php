...
$result  = move_uploaded_file($tempFile, "../img/" . $nameoffile);

if ($result) 
{
	//$query = "INSERT INTO images(path,uploadTime,included) VALUES ('$nameoffile',curdate(),false)";
	//$status = mysql_query($query);
	
	$stmt = $link->prepare("INSERT INTO images(path,uploadTime,included) VALUES (?,curdate(),false)");
	$stmt->bind_param('s',$nameoffile); 
	$stmt->execute();
